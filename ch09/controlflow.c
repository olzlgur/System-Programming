...
enum states	{NEUTRAL, WANT_THEN, THEN_BLOCK, WANT_ELSE}; // WANT_ELSE를 추가하였습니다.
 ...
 */
{
	...
	if( strcmp(cmd, "if") == 0) {
		if( if_state != NEUTRAL )
			...
			if(if_result==SUCCESS)    // if_result가 success이면 조건문을 만족했다는 뜻이므로 if_state를 WANT_THEN으로
     		   if_state=WANT_THEN; 
 	      	else                      // if_result가 success가 아니면 조건문을 만족시키지 못했다는 뜻이므로 if_state를 WANT_ELSE로 변경해준다.
 	     	   if_state=WANT_ELSE;
			rv = 0;
		}
	}
	   else if(strcmp(cmd, "else")==0) {  // else가 입력되었을 경우
  	     if(if_state!=WANT_ELSE)          // if_state가 WANT_ELSE가 아니면 else가 입력되면 안되는 상황이므로
 	       rv = syn_err("else unexpected"); // 에러를 띄워준다.
 	     if(if_result == FAIL){  // if_state가 WANT_ELSE이면서 if_result가 FAIL 이면 else가 충족되는 상황이므로
  	       if_state = WANT_THEN;  // if_state를 WANT_THENK으로 if_result를 SUCCESS로 변경해준다.
		   if_result = SUCCESS;  
 		  }
 	}
